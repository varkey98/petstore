...
public class PetController {
    ...
    public boolean createPet(@Argument int id, @Argument String name, @Argument DoctorInput doctor, @Argument PetOwnerInput owner) {
        try {
            Doctor doctorObject = doctorRepository.findOne(doctor.getId()).orElse(doctorRepository.create(doctor.getDoctorObject()));
            PetOwner ownerObject = ownerRepository.findOne(owner.getId()).orElse(ownerRepository.create(owner.getOwnerObject()));
    ...
    @MutationMapping
    public boolean updatePet(@Argument int id, @Argument String name, @Argument DoctorInput doctor, @Argument PetOwnerInput owner) {
        try {
            Pet pet = petRepository.findOne(id).get();
            if(name !=null) {
                pet.setName(name);
            ...
            if(doctor!=null){
                Doctor doctorObj = doctorRepository.findOne(doctor.getId()).orElse(doctorRepository.create(doctor.getDoctorObject()));
                pet.setDoctor(doctorObj);
            }

            if(owner!=null) {
                PetOwner ownerObj = ownerRepository.findOne(owner.getId()).orElse(ownerRepository.create(owner.getOwnerObject()));
